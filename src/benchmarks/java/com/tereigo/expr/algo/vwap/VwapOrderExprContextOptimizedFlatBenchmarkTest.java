...
// Flat-AST evaluator variant of VwapOrderExprContextOptimizedBenchmarkTest - same setup, same
// expression, so throughput numbers are directly comparable. Uses FlatExprEvaluatorFactory
// (com.tereigo.expr.impl.experimental) instead of ExprEvaluatorFactory - the optimized (static
// context resolved upfront) AST is flattened before evaluation.
public class VwapOrderExprContextOptimizedFlatBenchmarkTest {
    ...
    public static class BenchmarkState {
        ...
        public void prepare() {
            ...
            evaluator = FlatExprEvaluatorFactory.create(ctx, "(vwap.volumeLimit == 0.1) and (vwap.ric == 'VOD.L') and (order.ric == 'VOD.L') and (ric == 'VOD.L') and (vwap.tuid == 'CLIENT1') and (order.tuid == 'CLIENT1') and (tuid == 'CLIENT1') and (ric in ['BT.L', 'VOD.L', 'TSCO.L'])");
        }
    }
}
