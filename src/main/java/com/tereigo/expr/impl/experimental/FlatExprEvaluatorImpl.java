...
import com.tereigo.expr.impl.ExprContextNative;
...
/*
  Experimental: mirrors com.tereigo.expr.impl.ExprEvaluatorImpl, but flattens the compiled AST
  (see AstFlatter/FlatAST) before evaluating it.
 */
final class FlatExprEvaluatorImpl extends FlatExprEvaluatorBase implements ExprEvaluatorWithContext {

    FlatExprEvaluatorImpl(final ByteBuffer source) {
        this(ExprCompiler.compile(source));
    }

    FlatExprEvaluatorImpl(final ByteBuffer source, final Map<String, Expr.Literal> constants) {
        this(ExprCompiler.compile(source, constants));
    }

    FlatExprEvaluatorImpl(final String source) {
        this(ExprCompiler.compile(source));
    }

    FlatExprEvaluatorImpl(final String source, final Map<String, Expr.Literal> constants) {
        this(ExprCompiler.compile(source, constants));
    }

    private FlatExprEvaluatorImpl(final ASTRoot root) {
